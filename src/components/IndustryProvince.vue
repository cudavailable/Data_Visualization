	...
	const option = {
	  baseOption: {
		...
		visualMap: {
		  ...
		  max: 50000,
		  inRange: {
			color: ['#ffffff', '#a7d8de', '#4b9dd5', '#1d4e7d'] // 这是一个从白色到深蓝的示例
		  },
		  textStyle: {
			color: '#333' // 图例文字颜色
		  }
		...
		series: {
		  ...
		  label: {
			...
			show: function(params) {
			  // 只显示数值大于10000的标签
			  return params.data.value > 10000 || params.data.name === '东部' || 
					 params.data.name === '中部' || params.data.name === '西部' || 
					 params.data.name === '东北地区';
			},
			fontSize: 12,
			minAngle: 0,  // 当扇形角度小于15度时不显示标签
			color: 'grey' // 添加这行来设置标签文字颜色
		  },
		  emphasis: {
			focus: 'ancestor'
		  }
		...
		timeline: {
		  ...
		  playInterval: 1000,
		  label: {
			color: '#ffffff' // 时间轴文字颜色
		  },
		  lineStyle: {
			color: '#ffffff' // 时间轴线条颜色
		  },
		  itemStyle: {
			color: '#ffffff' // 时间轴节点颜色
		  },
		  controlStyle: {
			color: '#ffffff', // 控制按钮颜色
			borderColor: '#ffffff' // 控制按钮边框颜色
		  },
		  data: [
	  ...
	  options: [
		{ title: { text: '2013年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2013') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2014年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2014') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2015年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2015') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2016年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2016') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2017年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2017') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2018年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2018') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2019年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2019') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2020年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2020') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2021年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2021') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
		{ title: { text: '2022年各省工业增加值（亿元）' }, series: { data: buildSunburstData('2022') },textStyle: {color: '#ffffff'},subtextStyle: {color: '#ffffff'} },
	  ]
	};
